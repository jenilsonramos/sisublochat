...
export const evolutionApi = {
    ...
    createInstance: async (instanceName: string, token?: string, qrcode: boolean = true) => {
        ...
        try {
            // Auto-configure Webhook for this instance (FIRE AND FORGET)
            // Não esperamos o webhook configurar para liberar a UI
            try {
                console.log('Configuring Webhook (Async):', WEBHOOK_URL);
                api.post(`/webhook/set/${encodeURIComponent(instanceName)}`, {
                    webhook: {
                        enabled: true,
                        url: WEBHOOK_URL,
                        events: [
                ...
                }).catch(err => console.error('Webhook config failed (Async):', err));
            } catch (webhookError) {
                console.error('Failed to trigger webhook config:', webhookError);
            }
