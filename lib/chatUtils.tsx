...
let formatted: (string | JSX.Element)[] = [part];
...
formatted = formatted.flatMap(p => {
    if (typeof p !== 'string') return p;
    const parts = p.split(/```([^`]+)```/g);
    return parts.map((inner, i) => i % 2 === 1 ? <code key={`c-${index}-${i}`} className = "bg-black/20 px-1 rounded font-mono text-xs" > { inner } </code> : inner);
    });
