...
services:
  # O Backend (Node.js) que recebe os Webhooks para salvar mensagens
  app_backend:
    build: 
      context: ./server
...
conn.on('ready', () => {
    ...
    // Command to update compose file and list docker networks
    const cmds = [
        `echo '${newCompose}' > /root/evolutionapi/docker-compose.prod.yml`,
        "docker network ls",
        "ls -la /root/evolutionapi/supabase/functions"
    ];
    ...
    conn.exec(cmds.join(' && '), (err, stream) => {
        ...
        stream.on('close', (code, signal) => {
            console.log('---- DOCKER NETWORKS & FUNCTIONS ----');
