...
async function debugChats() {
    try {
        ...
        chats.slice(0, 3).forEach((c, idx) => {
            ...
            const msgContent = c.conversation || (c.message && c.message.conversation) || (c.message && c.message.extendedTextMessage && c.message.extendedTextMessage.text) || 'Unknown';
            console.log(`Last Msg: ${msgContent.substring(0, 50)}...`);
        });
